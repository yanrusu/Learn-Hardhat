...
describe("Bank test", function () {
  it("deposit & withdraw with correct mapping update", async function () {
    ...
    const bank = await ethers.getContractFactory("SimpleBank1");
    const Bank = await bank.deploy(await Token.getAddress(), await B_token.getAddress());
    ...
    console.log("B_token",await B_token.balanceOf(owner.address));
